...
curl -LO "VERRAZANO_DOWNLOAD_URL"
...
OK_CHECKSUM=`echo $CHECKSUM_OUTPUT | grep -v OK | wc -l`
if [ "$OK_CHECKSUM" -gt 0 ]
then
...
OK_VERSION=`echo $CHECKSUM_OUTPUT | grep -v $VERRAZZANO_VERSION | wc -l`
if [ "$OK_CHECKSUM" -gt 0 ]
then
...
echo "$VERRAZZANO_INSTALLED_VAR""=true"