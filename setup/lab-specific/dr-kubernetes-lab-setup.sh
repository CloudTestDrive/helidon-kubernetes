...
echo "  Create two Kubernetes clusters"
echo "  Create an auth token to use when talking to OCIR"
echo "  Create OCIR repos for the storefront, stockmanager, and logger microservices"
...
echo "  Start core Kubernetes services (Ingress contrtoller, Kubernetes dashboard) in both clusters"
echo "  Create service certificates and associated secrets based on Ingress load balancer IP in both clusters"
echo "  Create ingress rules, secrets and config maps based on the above info in both clusters"
echo "  Start four microservcies (storefront, stockmanager, logger and zipkin) in both clusters"
...
if [ "$RESP" -ne 0 ]
then
  echo "Core dual Kubernetes clusters setup module returned an error, unable to continue"
  exit $RESP
fi
...
if [ "$RESP" -ne 0 ]
then
  echo "Dual cluster Kubernetes services setup module returned an error, unable to continue"
  exit $RESP
fi
