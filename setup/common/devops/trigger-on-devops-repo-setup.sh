...
DEVOPS_TRIGGER_OCID=`oci devops trigger create-devops-code-repo-trigger --actions "$TRIGGER_ACTIONS" --display-name "$DEVOPS_TRIGGER_NAME" --project-id "$DEVOPS_PROJECT_OCID" --repository-id "$DEVOPS_REPO_OCID" --description "$DEVOPS_TRIGGER_DESCRIPTION" --wait-for-state "SUCCEEDED" --wait-interval-seconds 5 | jq -j '.data.resources[0].identifier'`
if [ -z "$DEVOPS_TRIGGER_OCID" ]
then
