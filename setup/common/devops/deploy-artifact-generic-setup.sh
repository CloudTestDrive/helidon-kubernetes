...
then
  ...
  then
    REPLY="y"
    echo "Devops generic deploy artifact $DEPLOY_ARTIFACT_NAME in devops project $DEVOPS_PROJECT_NAME exists, do you want to reuse it ? defaulting to $REPLY"
  else
    read -p "Devops  generic deploy artifact $DEPLOY_ARTIFACT_NAME in devops project $DEVOPS_PROJECT_NAME exists, do you want to reuse it ? (y/n) ? " REPLY  
  fi
  if [[ ! $REPLY =~ ^[Yy]$ ]]
  then
    echo "OK, you will need to delete the  generic deploy artifact $DEPLOY_ARTIFACT_NAME in devops project $DEVOPS_PROJECT_NAME"
    exit 1
  else
    echo "OK, will reuse existing devops  generic deploy artifact $DEPLOY_ARTIFACT_NAME in devops project $DEVOPS_PROJECT_NAME"
    echo "$DEPLOY_ARTIFACT_OCID_NAME=$DEPLOY_ARTIFACT_OCID" >> $SETTINGS
    echo "$DEPLOY_ARTIFACT_REUSED_NAME=false" >> $SETTINGS
    exit 0
  fi
