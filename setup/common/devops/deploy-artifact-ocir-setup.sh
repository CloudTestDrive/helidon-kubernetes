...
then
  ...
  DEVOPS_DEPLOY_ARTIFACT_DESCRIPTION="DOCKER_IMAGE"
fi
if [ $# -ge 5 ]
then
  ARTIFACT_TYPE="$5"
...
DEPLOY_ARTIFACT_OCID_NAME=`bash ./get-deplpy-artifact-ocid-name.sh $DEPLOY_ARTIFACT_NAME $DEVOPS_BUILD_PIPELINE_NAME $DEVOPS_PROJECT_NAME`
