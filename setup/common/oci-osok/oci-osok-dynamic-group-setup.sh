...
FINAL_RESP="0"
bash ./dynamic-group-instances-in-compartment-setup.sh "$USER_INITIALS"OCIOSOKDynamicGroup "This dynamic group identifies the resource in compartment $COMPARTMENT_NAME for user $USER_INITIALS to enable the oracle service operator for Kubernrtes to function"
RESP=$?
if [ "$RESP" -ne 0 ]
then
  echo "Problem setting up dynamic group "$USER_INITIALS"OCIOSOKDynamicGroup response is $RESP"
  FINAL_RESP=$RESP
fi
cd $SAVED_DIR
