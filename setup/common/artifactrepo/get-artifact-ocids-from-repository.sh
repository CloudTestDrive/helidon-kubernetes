...
oci artifacts generic artifact list --compartment-ocid $COMPARTMENT_OCID --repository-id $ARTIFACT_REPO_OCID $ARTIFACT_PATH_PARAM  $ARTIFACT_VERSION_PARAM | jq -r '.data.items[] | select (."lifecycle-state" != "DELETED") | .id'