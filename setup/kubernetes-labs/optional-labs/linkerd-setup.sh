...
echo "Remember to edit the linkerd visuals deployment and clean out the enforced-host element"
...
cd $HOME/helidon-kubernetes/service-mesh
$SMALLSTEP_DIR/step certificate create linkerd.$EXTERNAL_IP.nip.io tls-linkerd-$EXTERNAL_IP.crt tls-linkerd-$EXTERNAL_IP.key --profile leaf  --not-after 8760h --no-password --insecure --kty=RSA --ca $SMALLSTEP_DIR/root.crt --ca-key $SMALLSTEP_DIR/root.key
