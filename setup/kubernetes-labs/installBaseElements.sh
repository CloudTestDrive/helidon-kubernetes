...
then
  REPLY="y"
  echo "Auto confirm enabled, setting up config in downloaded git repo $currentContext is the kubernetes current context name default to $REPLY"
else
...
then
  ...
  echo "Fully install cluster, $currentContext is the kubernetes cluster name"
fi
