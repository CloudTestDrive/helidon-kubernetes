...
if [ $# -eq 1 ]
  then
    ...
    echo "Skipping stockmanager department setup confirmation using $department as the department name"
fi
config=$HOME/helidon-kubernetes/configurations/stockmanagerconf/conf/stockmanager-config.yaml
temp="$config".tmp
echo Updating the stockmanager config in $config to set $department as the department name
